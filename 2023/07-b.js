...
const handType = (cards) => {
    ...
    if(found.has('J')) {
        ...
        let highestCount = 0;
        let highestCard = "";
        let jokerCount = found.get('J');
        found.delete('J');
        found.forEach((count, card) => {
            if(highestCount < count) {
                highestCard = card;
                highestCount = count;
            }
        });
        found.set(highestCard, highestCount + jokerCount);
    }
    switch(found.size) {
         //five of a kind
...
const handTotal = (cards) => {
    ...
    for(i = 1; i <= cards.length; i++) {
        ...
        let cardValue = cardText.replace("T","10").replace("J","1").replace("Q","12").replace("K","13").replace("A","14") * 1;
